...
class SnappyCompressFromIOVecTester:
    def __init__(self):
    ...
    def test_iovec_compression(self, data_buffers, total_size):
        try:
            compressed_iovec = self.snappy.compress_from_iovec(data_buffers)
    ...
    def compare_with_regular_compression(self, expected_data, uncompressed, total_size):
        ...
        if uncompressed is not None:
            compressed_iovec_size = len(self.snappy.compress_from_iovec([expected_data]))
            print(f"Size difference (IOVec - Regular): {compressed_iovec_size - len(regular_compressed)} bytes")
    ...
    def run_all_tests(self):
        ...
        compressed_iovec, uncompressed, expected_data = self.test_iovec_compression(data_buffers, total_size)
        
        if expected_data is not None:
            self.compare_with_regular_compression(expected_data, uncompressed, total_size)
        
        self.run_edge_case_tests()


def main():
    tester = SnappyCompressFromIOVecTester()
